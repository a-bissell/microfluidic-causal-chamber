...
def build_pipeline(vtk_file, w_um, x_lo, x_hi, y_stub):
    ...
    # multiphaseInterFoam carries three water phases, not one alpha.water.
    # The interface to contour is total water = sum of the three; the code
    # is their RATIO, coloured onto that surface below.
    cd = grid.GetCellData()
    w = [numpy_support.vtk_to_numpy(cd.GetArray(f"alpha.water{i}")) for i in (1, 2, 3)]
    tot = w[0] + w[1] + w[2]
    aw = numpy_support.numpy_to_vtk(np.ascontiguousarray(tot), deep=1)
    aw.SetName("alpha.water")
    grid.GetCellData().AddArray(aw)
    ...
    # Colour the interface by COMPOSITION: dye1=R, dye2=G, dye3=B, from the
    # three water phases interpolated onto the surface. This is the encoder's
    # whole point -- the code carried inside the droplet -- made visible in 3D,
    # where the mill renderer showed only solid-blue water.
    final = normals.GetOutput()
    pdd = final.GetPointData()
    pw = [numpy_support.vtk_to_numpy(pdd.GetArray(f"alpha.water{i}")) for i in (1, 2, 3)]
    ptot = np.maximum(pw[0] + pw[1] + pw[2], 1e-6)
    comp = np.stack([pw[0]/ptot, pw[1]/ptot, pw[2]/ptot], axis=1)
    # gamma < 1 lifts the mixed midtones so a 1/3-1/3-1/3 patch reads as a warm
    # grey rather than near-black, while pure laminae stay saturated.
    rgb = np.clip(comp ** 0.75, 0, 1)
    rgb8 = np.ascontiguousarray((rgb * 255).astype(np.uint8))
    colors = numpy_support.numpy_to_vtk(rgb8, deep=1, array_type=vtk.VTK_UNSIGNED_CHAR)
    colors.SetName("code"); colors.SetNumberOfComponents(3)
    final.GetPointData().SetScalars(colors)

    dm = vtk.vtkPolyDataMapper()
    dm.SetInputData(final)
    dm.SetColorModeToDirectScalars()
    dm.ScalarVisibilityOn()
    drop = vtk.vtkActor()
    drop.SetMapper(dm)
    pr = drop.GetProperty()
    pr.SetSpecular(0.5); pr.SetSpecularPower(40)
    pr.SetDiffuse(0.85); pr.SetAmbient(0.22)
...
def main():
    ...
    # foamToVTK writes a .vtk.series mapping each file to its exact time --
    # authoritative, and needed here because the reconstructed root time-dirs
    # were cleaned to save disk. Fall back to the shared resolver otherwise.
    import json as _json
    series = list((args.case_dir / "VTK").glob("*.vtk.series"))
    if series:
        m = {e["name"]: e["time"] for e in _json.loads(series[0].read_text())["files"]}
        times = np.asarray([m.get("_" + f.stem.split("_")[-1], np.nan) for f in files])
    else:
    ...
    fig.suptitle(f"{args.w_main_um:.0f} µm encoder — coded droplet forming in 3D\n"
                 "surface coloured by composition (R=dye1 G=dye2 B=dye3); "
                 "half-depth mirrored to full channel", fontsize=12)
