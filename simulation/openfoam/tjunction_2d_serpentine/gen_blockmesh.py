...
Run:  python3 gen_blockmesh.py            (writes system/blockMeshDict at DX=7.5 um,
                                            L_OUTLET=1037.5 um, the committed/
                                            published resolution and geometry)
      ...
      python3 gen_blockmesh.py --l-outlet 3000   (mesh_convergence_2026-07
                                            found some high-throughput cases
                                            (endTime=0.12s, P_disp=3.6 kPa)
                                            filling the default 1037.5 um
                                            outlet with queued droplets,
                                            confounding the droplet-length
                                            measurement; a longer outlet
                                            gives them room to separate)
"""
import argparse
...
BASE_L_OUTLET = 1037.5    # outlet length the base NX below was tuned for
...
_args.add_argument("--l-outlet", type=float, default=BASE_L_OUTLET,
                    help="Outlet channel length (um) past the junction; "
                         "default reproduces the committed mesh exactly.")
_parsed = _args.parse_args()
DX = _parsed.dx
L_OUTLET = _parsed.l_outlet
...
XS = [-L_RES_OIL, 0.0, 537.5, 560.0, 590.0, 612.5, 612.5 + L_OUTLET]
...
# design. simpleGrading ratios (below) are resolution-independent. The
# outlet interval's cell count is instead recomputed from L_OUTLET directly
# (not scaled from a fixed base count) so --l-outlet changes only the
# outlet's physical length, not its cell density.
_scale = BASE_DX / DX
NX = [max(1, round(n * _scale)) for n in (50, 72, 3, 4, 3)]
NX.append(max(1, round(L_OUTLET / DX)))
